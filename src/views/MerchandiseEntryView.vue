...
import { setAuthorization, setAuthorizationHeader, usePost } from '@/composables/useRequest'
import {
...
const formData = reactive<MerchandiseEntry>({
  name: '',
  barcode: '',
  price: 0,
  picture_url: '',
...
async function onSubmit() {
  ...
  const { data, error } = await usePost('/merchandise/create', {
    data: {
      ...formData
    },
...
const onBarcodeScanned = (barcode: Barcode) => {
  formData.barcode = barcode.displayValue
}
...
const afterUploaderRead = async (file: any) => {
  const { data, error } = await usePost('/merchandise/upload-picture/' + formData.barcode, {
    headers: {
  ...
  if (get(error)) {
    ...
    formData.picture_url = get(data).data
  }
}

const onShellLifePickerConfirm = ({ selectedValues }: { selectedValues: number[] }) => {
  formData.shilf_life = selectedValues[0]
...
const onSpecsPickerConfirm = ({ selectedValues }: { selectedValues: string[] }) => {
  formData.specs = selectedValues[0]
